...
- $f(\mathbf{x}) = \frac{1}{2\pi \sigma_1 \sigma_2 \sqrt{1 - \rho^2}} \exp\left( -\frac{1}{2(1 - \rho^2)} \left[ \frac{(x_1 - \mu_1)^2}{\sigma_1^2} + \frac{(x_2 - \mu_2)^2}{\sigma_2^2} - \frac{2\rho(x_1 - \mu_1)(x_2 - \mu_2)}{\sigma_1 \sigma_2} \right] \right)$
- $F(\mathbf{x}) = P(X_1 \le x_1, X_2 \le x_2) = \int_{-\infty}^{x_2}\int_{-\infty}^{x_1}f(x_1, x_2)dx_1 dx_2$
:::
...
- $f(\mathbf{x}) = f(x_1, \dots, x_p) = \frac{1}{(2\pi)^{p/2}|\Sigma|^{1/2}}\exp\left(-\frac{1}{2}(\mathbf{x} - \mathbf{\mu})^T\mathbf{\Sigma}^{-1}(\mathbf{x}-\mathbf{\mu})\right)$
:::
...
וזה אומר האי-שווין שלפנינו. גם כאן נשים לב שיש ביטויים מיותרים (להדגים), אם נפתח את הסוגריים נקבל בשני האגפים את הביטוי xSigmax ואפשר להשמיט אותו משני האגפים.
