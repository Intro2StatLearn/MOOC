...
- $f(\mathbf{x}) = f(x_1, \dots, x_p) = \frac{1}{(2\pi)^{p/2}|\mathbf{\Sigma}|^{1/2}}\exp\left(-\frac{1}{2}(\mathbf{x} - \mathbf{\mu})^T\mathbf{\Sigma}^{-1}(\mathbf{x}-\mathbf{\mu})\right)$
:::
...
### LDA: $k-j$ classes decision rule (I)
...
### LDA: $k-j$ classes decision rule (II)
