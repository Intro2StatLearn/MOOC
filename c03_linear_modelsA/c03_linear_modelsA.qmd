...
ואם p = 0.3, אז יוצא ביטוי אחר. בכל מקרה ברור שאם הסיכוי לקבל את המדגם שקיבלתי תחת ההנחה שp = 0.3 גדול יותר מהסיכוי לקבל את המדגם תחת ההנחה שp = 0.2, נראה הערך 0.3 סביר יותר.
...
ההסתברות של המדגם שלנו היא ההסתברות של תוצאה בינומית שרשומה כאן, ונגדיר שהיא פונקציה של p שנקרא לה נראות, לייקליהוד. מסמנים בדרך כלל עם L גדולה כפונקציה של p בהינתן המדגם שהתקבל. זה בעצם ההסתברות לקבל את המדגם כפונקציה של p.
...
::: {.incremental}
- Second generalization, when $X_1, \dots, X_n \sim P_\theta$ iid
...
::: {.notes}
::: {style="direction:rtl; font-size:16px"}
אז כאמור X הוא לא משתנה מקרי תחת מודל הרגרסיה, רק אפסילון הרעש משתנה מקרי. יוצא מזה שY הוא טרנספורמציה ליניארית של משתנה מקרי, ולכן הוא בעצמו משתנה מקרי שמתפלג נורמלית, עם תוחלת שהיא קו הרגרסיה האמיתי, ושונות סיגמה בריבוע.
...
::: {.notes}
::: {style="direction:rtl; font-size:16px"}
נתחיל ברגרסיה פשוטה עם משתנה אחד, שם השאלה הטבעית היא מה השורה התחתונה המשתנה קשור לY, משפע עליו, או לא. כמו תמיד, יש כאן אלמנט של ספק תמיד, והשאלה היא האם השתכנענו מעל לספק סביר שהמשתנה משפיע על Y. אנחנו תמיד לוקחים בחשבון את האפשרות שאנחנו טועים.
...
::: {.notes}
::: {style="direction:rtl; font-size:16px"}
מה שמביא אותנו לדיון קצר על פיצ'ר סלקשן, בחירת משתנים, נושא שנעסוק בו בהמשך. איך נזהה את תת הקבוצה הטובה ביותר? היא לא קטנה כל כך שהמודל פשטני מדי עם שאריות גדולות ועם R בריבוע קטן. והיא לא גדולה מדי, וסתם מנפחת את R בריבוע.
...
מלבד הגישות האלה יש עוד שאולי ניתקל בהן בהמשך הקורס. עד כאן לגבי רגרסיה ליניארית. בשיעור הבא נראה איך להגמיש את המודל הליניארי, ולהתאים אותו גם למצב שY הוא משתנה קטגוריאלי.
:::
:::
