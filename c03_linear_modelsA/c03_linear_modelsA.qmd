...
- First generalization:
  ...
  - Notice with: $\text{Log-Likelihood}(p) = \ell(p|x_1, \dots, x_n) = \log P(x_1, \dots, x_n) =$
  $\approx k\log p + (n -k)\log(1-p)$
...
- Covariance matrix: 
$$Cov(\hat{\beta}) \stackrel{(c)}{=} (X^TX)^{-1} X^T Cov(y) X (X^TX)^{-1} \stackrel{(b)}{=} \sigma^2 (X^TX)^{-1} (X^T X) (X^TX)^{-1}$$
$$= \sigma^2 (X^TX)^{-1}$$
:::
