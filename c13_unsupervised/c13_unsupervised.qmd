...
::: {.incremental}
- Minimizing "within cluster" scatter/loss:
...
אבל אנחנו יודעים כבר שאם נסתכל על קריטריון דומה, ונשאל מה הנקודה שמביאה למינימום את סכום המרחקים המרובעים ממנה -- נגיע לממוצע המדגם.
...
:::
::: {.fragment}
$$\begin{align}w &= \arg\min_{w: \|w\|=1}\|X - \hat{X}\|^2_F \\ &= \arg\min_{w: \|w\|=1}\text{Tr}((X - Xww^T)(X - Xww^T)^T)\end{align}$$
:::
...
עם המוטיבציה שהראינו כאן אגב, אפשר להבין למה חוקרים מסוימים מסתייגים מהמונח למידה אנסופרוויזד ומעדיפים להשתמש במונח למידה סלף-סופרוויזד, כלומר זה לא שאין מפקח בכלל, הדאטה הוא המפקח של עצמו, כרגע מצאנו דרך לתמצת ולשחזר אותו על-פי קריטריון ריבועי.
:::
:::
...
$$2X^TXw_1 - 2\lambda_1w_1 = \mathbf{0} \Rightarrow X^TXw_1 = \lambda_1w_1$$
...
# Generate random clustered data points in 3D
np.random.seed(0)
n_points = 100

# Create points from three Gaussian clusters in 3D
cluster_1 = np.random.randn(n_points, 3) + np.array([5, 0, 5])
cluster_2 = np.random.randn(n_points, 3) + np.array([0, 5, -5])
cluster_3 = np.random.randn(n_points, 3) + np.array([5, 5, 5])
X = np.vstack([cluster_1, cluster_2, cluster_3])

# Apply t-SNE to project the 3D data to 2D
...
# 3D scatter plot (left) to resemble a "cloud"
ax1 = fig.add_subplot(121, projection='3d')
ax1.scatter(X[:, 0], X[:, 1], X[:, 2], color='red', alpha=0.5)
ax1.set_xlabel('X1')
ax1.set_ylabel('X2')
ax1.set_zlabel('X3')
ax1.set_title("3D X Data")

# 2D scatter plot of t-SNE projection (right) showing cluster structure
ax2.scatter(X_2d[:, 0], X_2d[:, 1], color='red', alpha=0.5)
ax2.set_xlabel('Y1')
ax2.set_ylabel('Y2')
ax2.set_title("2D Y t-SNE Projection")
...
אז שוב u_i הוא וקטור מקרי ממימד נמוך q מתוך התפלגות נורמלית, עם תוחלת אפס ומטריצת שונות שהיא מטריצת היחידה. כלומר מטריצת אפסים עם אחת על האלכסון. לוקטור כזה קוראים הרבה פעמים וקטור לטנטי או חבוי, וכך גם לכל המימד הנמוך הזה, המרחב הלטנטי, כלומר אנחנו מניחים שכל שורה במימד שאנחנו רואים, מימד האאוטפוט במימד גבוה, נוצרה מתוך מרחב חבוי ממימד נמוך יותר.

W היא מטריצה ממשית בגודל p על q של פרמטרים שצריך למצוא, והיא קבועה לכל המודל. מיו הוא וקטור תוחלות באורך פי. ואפסילון-איי הוא וקטור מקרי באורך p שמייצג רעש אקראי מהתפלגות נורמלית עם איזשהו פרמטר סיגמה בריבוע שנצטרך לאמוד.
...
$$\hat{W}_{MLE} = \mathbf{V}_q(\mathbf{\Lambda}_q - \sigma^2I_q)^{1/2}\mathbf{R}$$
...
- PCA as a generative model: $\mathbf{x}_* = \hat{W}\mathbf{u}_* + \hat{\mathbf{\mu}}$
:::
