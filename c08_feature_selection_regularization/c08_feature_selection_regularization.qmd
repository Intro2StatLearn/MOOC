...
for k in range(X.shape[1] + 1):
    ...
    plt.scatter(k_rss['k'], np.log(k_rss['RSS']), color='blue', label='All models' if k == 0 else "")
best_rss_df = pd.DataFrame(best_models, columns=['k', 'RSS', 'Features'])
plt.plot(best_rss_df['k'], np.log(best_rss_df['RSS']), color='red', label='Best models')
plt.plot(best_rss_df['k'], np.log(rss_plus_penalty), color='green', linestyle='dashed', label='Best models + Cp penalty')
...
### Choosing the $\lambda$ [hyperparameter]{style="color:red;"}
...
For Gaussian distribution, if $X \sim \mathcal{N}(\mu_X, \sigma^2_X), Y \sim \mathcal{N}(\mu_Y, \sigma^2_Y), \rho = Corr(X, Y)$:
...
# Generate prior distribution for Y
y_vals = np.linspace(mu_Y - 4 * sigma_Y, mu_Y + 4 * sigma_Y, 500)
prior_Y = np.exp(-0.5 * ((y_vals - mu_Y) / sigma_Y) ** 2) / (sigma_Y * np.sqrt(2 * np.pi))

# Plot setup
fig, axs = plt.subplots(1, 3, figsize=(5 * 3, 3.5))

# Left panel: Prior distribution of Y
axs[0].plot(y_vals, prior_Y, color='black')
axs[0].set_title('Prior of Y (and X)')

# For the conditional distributions
x_obs = 1  # Observed value of X

for i, rho in enumerate(rho_values):
    # Covariance matrix
    cov = np.array([[sigma_Y ** 2, rho * sigma_Y * sigma_X],
                    [rho * sigma_Y * sigma_X, sigma_X ** 2]])
    
    # Conditional mean and variance for Y given X = x_obs
    conditional_mu_Y = mu_Y + rho * (sigma_Y / sigma_X) * (x_obs - mu_X)
    conditional_var_Y = (1 - rho ** 2) * sigma_Y ** 2
    conditional_sigma_Y = np.sqrt(conditional_var_Y)
    
    # Generate conditional distribution of Y given X
    conditional_Y = np.exp(-0.5 * ((y_vals - conditional_mu_Y) / conditional_sigma_Y) ** 2) / (conditional_sigma_Y * np.sqrt(2 * np.pi))
    
    # Plot the conditional distribution in the middle and right panels
    axs[i+1].plot(y_vals, conditional_Y, color='black')
    axs[i+1].set_title(f'Conditional Y | X = {x_obs}, ρ = {rho}')
